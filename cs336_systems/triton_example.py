...
def weighted_sum_fwd(
    ...
    # Our grid is only 1D, so we just take the first dimension.
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        # Reshape input tensor to 2D to simplify pointer arithmetic (without
        # really moving any data) as we can ignore how many dimensions of
        # batches therea are.
        input_shape = x.shape
        x = rearrange(x, "... d -> (...) d")
